...
pub(super) async fn send(
    ...
    sqlx::query("WITH expired AS (SELECT id FROM secret_messages WHERE expires_at<=now() AND ciphertext IS NOT NULL ORDER BY expires_at LIMIT 1000 FOR UPDATE SKIP LOCKED) UPDATE secret_messages SET ciphertext=NULL FROM expired WHERE secret_messages.id=expired.id")
    ...
    for account in [user, body.recipient_id] {
        let (count,bytes):(i64,i64)=sqlx::query_as("SELECT count(*),COALESCE(sum(octet_length(ciphertext)),0)::bigint FROM secret_messages WHERE ((sender_id=$1 AND NOT sender_deleted) OR (recipient_id=$1 AND NOT recipient_deleted)) AND (expires_at IS NULL OR expires_at>now()) AND ciphertext IS NOT NULL")
            .bind(account).fetch_one(&mut *tx).await?;
        if count >= 1000 || bytes + body.ciphertext.len() as i64 > 20 * 1024 * 1024 {
            return Err((StatusCode::TOO_MANY_REQUESTS, "Mailbox quota exceeded").into());
        }
    }
    let row:StoredMessage=sqlx::query_as("INSERT INTO secret_messages(id,sender_id,recipient_id,ciphertext,ciphertext_hash,sender_public_key,recipient_public_key,expires_at) VALUES($1,$2,$3,$4,$5,(SELECT public_key FROM users WHERE id=$2),(SELECT public_key FROM users WHERE id=$3),$6) RETURNING *")
        .bind(body.id).bind(user).bind(body.recipient_id).bind(&body.ciphertext).bind(hash(&body.ciphertext)).bind(body.expires_at).fetch_one(&mut *tx).await?;
    tx.commit().await?;
...
pub(super) async fn delete(
    ...
    let result=sqlx::query("UPDATE secret_messages SET sender_deleted=sender_deleted OR sender_id=$2,recipient_deleted=recipient_deleted OR recipient_id=$2,ciphertext=CASE WHEN (sender_deleted OR sender_id=$2) AND (recipient_deleted OR recipient_id=$2) THEN NULL ELSE ciphertext END WHERE id=$1 AND (sender_id=$2 OR recipient_id=$2)")
